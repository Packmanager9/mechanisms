...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    let buffer = {}
    let powergear = new Image()
    powergear.src = 'powergear4.png'
    let rampshot = new Image()
    rampshot.src = 'rampshot.png'
    let walldulum = new Image()
    walldulum.src = 'wallsheet.png'
    ...
    function setUp(canvas_pass, style = "#000000") {
        ...
        window.addEventListener('pointerdown', e => {
            ...
            buffer.go = 1
            buffer.x = TIP_engine.x
            buffer.y = TIP_engine.y
        });
...
//         console.log(gamepadAPI.axesStatus[1]*gamepadAPI.axesStatus[0]) //debugging
    ...
    function getRandomGreen() { // random color
        var letters = '0123456789ABCDEF';
        var color = '#00';
        for (var i = 0; i < 2; i++) {
    ...
    class Player{
        ...
        shoot(){
            let point = new Circle(this.body.x, this.body.y, 10, "red")
            gamepad_control2(point,this.speed)
            let link = new LineOP(point, this.body)
            if(link.hypotenuse()>(this.speed*.5)){
                this.shooting++
                if(this.shooting%this.rate == 0){
                    let shot = new Circle(point.x, point.y, 5, "black", Math.cos(link.angle())*this.speed*2, Math.sin(link.angle())*this.speed*2)
                    shot.life = 20
        ...
        draw(){
            gamepad_control(this.body,this.speed)
            this.shoot()
            this.body.draw()
            for(let t = 0;t<this.shots.length;t++){
                ...
                if(this.shots[t].bead == 1 && this.shots[t].radius > .1){
                    this.shots[t].radius*=.9
                }   
                ...
                for(let k = 0;k<enemies.length;k++){
                    if(this.shots[t].doesPerimeterTouch(enemies[k])){
                        enemies[k].health-= (Math.abs(this.shots[t].xmom)+Math.abs(this.shots[t].ymom))*this.shots[t].radius
                        enemies[k].xmom += (this.shots[t].xmom*this.shots[t].radius)/5
                        enemies[k].ymom += (this.shots[t].xmom*this.shots[t].radius)/5
                        if(this.shots[t].bead != 1){
                            for(let r = 0;r<9;r++){
                                let bead = new Circle(this.shots[t].x, this.shots[t].y, 3, "black", Math.cos(((Math.PI*2)/9)*r)*this.speed, Math.sin(((Math.PI*2)/9)*r)*this.speed)
                                bead.bead = 1
                                this.shots.push(bead)
                            }
                            this.shots[t].bead = 1
                        }
                    }
    ...
    let enemies = []
    for(let t = 0;t<1;t++){
        let enemy = new Circle(500, 350+t*10, 20, "red")
        enemies.push(enemy)
    }
    let player = new Player(350,350)
    let rate = 100
    let start = 0
    ...
    class Stacker {
        constructor(){
            this.width = 150
            this.blocks = [new Rectangle(0,715, 720,1000, "red")]
            this.block = new Rectangle(300,40, this.width,40, "red")
            this.beat = 0
            this.speed = .01
            this.timelock = 0
        }
        draw(){
            ...
            if(this.block.y+this.block.height >= this.blocks[this.blocks.length-1].y){
                let xdif = this.block.x-this.blocks[this.blocks.length-1].x
                if(xdif < 0){
                    let xgap = this.block.width+xdif
                    this.width =  Math.min(Math.min(Math.abs(xgap), this.blocks[this.blocks.length-1].width),this.width)
                }else{
                    let xgap = this.blocks[this.blocks.length-1].width-xdif
                    this.width =  Math.min(Math.min(Math.abs(xgap), this.block.width),this.width)
                }
                if(this.block.x < this.blocks[this.blocks.length-1].x){
                    this.block.width -= (this.blocks[this.blocks.length-1].x-this.block.x)
                    this.block.x = this.blocks[this.blocks.length-1].x
                }
                if(this.block.x+this.block.width > this.blocks[this.blocks.length-1].x+this.blocks[this.blocks.length-1].width){
                    let xdif =  ((this.block.x-this.blocks[this.blocks.length-1].x))
                    let wdif =  ((this.block.width-this.blocks[this.blocks.length-1].width))
                    if(wdif-xdif < 0){
                        this.block.width += wdif-xdif
                    }
                }
                if(this.block.width < 0){
                    this.block.width = 0
                    this.width = 0
                }
            ...
            for(let t = 0;t<this.blocks.length;t++){
                this.blocks[t].y += Math.min(this.blocks.length*1, 40)
            }
                ...
                if(this.fired == 1){
                    ...
                    this.block.x = (Math.cos(this.beat)*200)+300
                    if(keysPressed[' ']){
                        this.fired = 1
                    }
                }
    ...
    class Grid {
        constructor(){
            ...
            for(let t = 0;t<5;t++){
                let row = []
                for(let k = 0;k<9;k++){
                    let block = new Rectangle(300+(k*100), 110+(t*100), 100,100, getRandomGreen())
                    row.push(block)
                }
            ...
            if(buffer.go == 1){
                ...
                for(let t = 0;t<5;t++){
                    ...
                    this.blocks[t][k].draw()
                        if(this.blocks[t][k].isPointInside(buffer)){
                            let mech = new Mechanism(this.blocks[t][k])
                            mechs.push(mech)
                        }
                    }
                }
    ...
    class Mechanism {
        ...
        draw(){
            ...
            // }
            if(this.type == 0){
                canvas_context.drawImage(powergear,(100*(this.step%6)), 0, 100,100,this.tile.x, this.tile.y, 100,100)
            }else if(this.type == 1){
                canvas_context.drawImage(rampshot,(100*(this.step%12)), 0, 99,100,this.tile.x, this.tile.y, 100,100)
                if(this.step%12 == 11){
                    let circ = new Circle(this.tile.x+90, this.tile.y+17, 10, "blue", 10)
                    this.shots.push(circ)
                }
            }else if(this.type == 2){
                canvas_context.drawImage(walldulum,(100*(this.step%26)), 0, 100,100,this.tile.x, this.tile.y, 100,100)
            }
            for(let t = 0;t<this.shots.length;t++){
                console.log(t)
                this.shots[t].move()
